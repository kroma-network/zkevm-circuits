...
mod sstore_tests {
    ...
    use mock::{
        test_ctx::helpers::{setup_kroma_required_accounts, tx_from_1_to_0},
    ...
    fn test_ok(is_warm: bool) {
        ...
        assert_eq!(
            ...
            [
                (
                    RW::READ,
                    &StackOp::new(expected_call_id, StackAddress::from(1022), Word::from(0x0u32))
                ),
                (
                    RW::READ,
                    &StackOp::new(expected_call_id, StackAddress::from(1023), Word::from(0x6fu32))
                ),
            ]
        );
