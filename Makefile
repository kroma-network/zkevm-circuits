...
clippy: ## Run clippy checks over all workspace members
	@cargo check
	@cargo clippy --all-targets -- -D warnings -Aclippy::format_in_format_args -Aclippy::uninlined_format_args -Aclippy::unnecessary_cast
...
test-light: ## Run light tests
	@cargo test --release --all --exclude integration-tests --exclude testool --exclude mock --exclude external-tracer --exclude circuit-benchmarks

test-heavy: ## Run heavy tests serially to avoid OOM
	@cargo test --release --features kroma --all --exclude integration-tests --exclude testool --exclude mock --exclude external-tracer --exclude circuit-benchmarks serial_ # -- --test-threads 1
...
.PHONY: clippy doc fmt test test_benches test-all help #evm_bench state_bench circuit_benches evm_exec_steps_occupancy stats_state_circuit stats_evm_circuit stats_copy_circuit
