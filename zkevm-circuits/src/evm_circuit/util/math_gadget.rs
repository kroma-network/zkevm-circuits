...
impl<F:FieldExt> ShlWordsGadget<F> {
	pub(crate) fn construct(
		...
		for transplacement in (0_usize)..(4_usize) {
			let select_transplacement_polynomial = generate_lagrange_base_polynomial(shift_div_by_64.clone(), transplacement as u64, 4 as u64);
			for idx in 0..(4 - transplacement) {
				let tmpidx = idx + transplacement;
				let merge_a = if idx == (0_usize) {
					a_slice_back_digits[idx].clone() * shift_mod_by_64_pow.expr()
				} else {
					a_slice_back_digits[idx].clone() * shift_mod_by_64_pow.expr() + a_slice_front_digits[idx - 1].clone()
				};
				shl_constraints[tmpidx] = shl_constraints[tmpidx].clone() +
					select_transplacement_polynomial.clone() * (merge_a - b_digits[tmpidx].clone());
			}
			for idx in 0..transplacement {
				shl_constraints[idx] = shl_constraints[idx].clone() + select_transplacement_polynomial.clone() * b_digits[idx].clone();
			}
		}
		for idx in 0..4 {
			cb.require_zero(
