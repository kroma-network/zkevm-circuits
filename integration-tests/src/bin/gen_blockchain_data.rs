use eth_types::kanvas_params::PROPOSER_FEE_RECIPIENT;
use ethers::{
    abi::{self, Tokenize},
...
use integration_tests::{
    get_provider, get_wallet, log_init, CompiledContract, GenDataOutput, CONTRACTS,
    CONTRACTS_PATH,
};
use log::{error, info};
...
async fn main() {
    ...
    let receipt = wallets[0]
        ...
        .unwrap();
    blocks.insert("Transfer 0".to_string(), receipt.block_number.unwrap().as_u64());
    ...
    .await;
    blocks.insert("Deploy Greeter".to_string(), receipt.block_number.unwrap().as_u64());
    deployments.insert(
        "Greeter".to_string(),
