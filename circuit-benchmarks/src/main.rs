...
// mod evm_circ_benches {
    ...
    fn main() {
        ...
        let mut file =
            File::create("&vk_path").expect("Failed to create sha256_vk");

        file.write_all(&buf[..])
            .expect("Failed to write vk to file");

        let vk_fs =
            File::open("&vk_path").expect("couldn't load sha256_params");
